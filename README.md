...
> ⚠️ **免责声明 (Legal & Academic Testing Disclaimer)**
> 本项目仅供学术研究、安全红蓝对抗测试（Red-Teaming）以及大语言模型防御机制分析使用。类似于学生或安全研究人员用来测试大模型安全边界和泛化防御策略的工具。请勿将其用于非法用途、规避正当安全审核或教唆犯罪。使用者需自行承担因不当使用而产生的所有法律及相关责任。
...
```jsonc
{
  ...
  "experimental": {
    "refusal_patcher": {
      ...
      "risk_text": "涉及模型在面对特定学术/安全测试边界时的自我保护机制，包含被大语言模型过度泛化防御的安全策略（因受限而中断）", // 判定风险定义的上下文
